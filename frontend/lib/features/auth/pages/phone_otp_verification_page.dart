...
class PhoneOtpVerificationPage extends StatefulWidget {
  const PhoneOtpVerificationPage({super.key});
...
class _PhoneOtpVerificationPageState extends State<PhoneOtpVerificationPage> {
  ...
  void initState() {
    ...
    _placeholderFuture = VerificationPagePlaceholder.fromAsset(
      path: 'assets/placeholders/phone_otp_verification_page.json',
    );
  }
  ...
  Future<void> _onVerify() async {
    ...
    final placeholder = await _placeholderFuture;
    if (_otpValue.length != placeholder.otp.length) return;
    setState(() => _isVerifying = true);
    try {
      await AuthService.instance.verifyPhoneOtp(_otpValue);
      if (!mounted) return;
  ...
  Future<void> _onResend() async {
    ...
    try {
      await AuthService.instance.resendPhoneOtp();
      if (!mounted) return;
      _showInfo('A new WhatsApp code was sent to your phone.');
    } on AuthException catch (e) {
      if (!mounted) return;
      _showError(e.message);
    } finally {
    ...
    return FutureBuilder<VerificationPagePlaceholder>(
      ...
      builder:
          (
            ...
            // Show the verified phone number in the header instead of placeholder email.
            final String phoneDisplay =
                RegistrationSession.instance.phone ?? data.header.email;
